...
|---|---|---|
| 2026-08-26 | **The message template is stored on the event row**, not looked up in a per-project vocabulary table | Grouping the top-messages widgets by a fingerprint needs a label for each group, and Postgres supplied it from `message_templates` — a table Phase 4 deletes. Three options. Join it: impossible across two stores. Display `any(message)`: labels a group of ten thousand with one arbitrary instance, which reads as a single event and is worse than the raw-text grouping it replaced. Store it: a third near-copy of the message text on every row. The third looked most expensive and is not — **measured at 2.00 bytes/row, 13× compression, 4.4% of the table**, against `message` at 4.85 and 5.8×, because 2,252 distinct templates against 110,112 distinct messages is exactly the case ZSTD is best at and the sort key puts near-identical values in one granule. The general point: a derived value computed by application code has **no** SQL equivalent, so "store it or lose it" is the whole of the choice — a normaliser written twice, once in TypeScript and once in SQL, is the drift this repository keeps paying for. |
| 2026-08-26 | **`topMessages` groups by `template_hash` always**, ending a split where the same widget answered two different questions depending on coverage | Under Postgres the rollup path grouped by the fingerprint and the fallback grouped `SUBSTRING(message, 1, 200)`. Those are not slow and fast versions of one query: the first says `User *** signed in` occurred 4,000 times, the second says four thousand things occurred once each. Which one a reader saw depended on whether a rollup covered the range and whether an environment filter was active — both invisible on screen, and nothing compared them. This is what a fallback *becomes* when it is allowed to answer a different question: not a safety net, a second product. Choosing the fingerprint is choosing the answer the widget was always titled for. **The rule to take from it:** two implementations of one read are acceptable only while a test asserts they return the same thing; the moment that test cannot be written, one of them is wrong. |
| 2026-08-26 | **The template fingerprint stops being folded into a signed range** — `toSignedBigint`, `toUnsignedBigint` and `templateHashForStorage` are deleted | They existed for one reason: Postgres' `bigint` is signed and an FNV-1a hash is not, so the same number was folded on the way in and unfolded on the way out. Neither fold is an involution, so applying one twice or forgetting it once produced a **silently different group key** rather than an error — the failure mode this codebase has the worst record against. ClickHouse's `UInt64` is the range the hash already lives in. Worth recording as a shape rather than a detail: a bijection maintained by hand across a boundary is a correctness obligation on every caller, and removing the boundary removes the obligation. |
| 2026-08-26 | **Two e2e suites in a row were found not to test the page they were named after**, and the gate wording that hid it is now distrusted by default | Phase 3's gate was "events + alerts e2e green" and Phase 4's was "dashboards e2e green". Six of nine `events.spec.ts` tests and six of eight `dashboard.spec.ts` tests queried the database directly and rendered nothing; the browser tests that remained checked that an `<h2>` existed and that a page contained the word "curl". Both suites would have stayed green with the feature completely broken. Phase 3 caught its own and **explicitly flagged that Phase 4's gate was phrased identically** — which is the only reason Phase 4's was checked rather than trusted. **A gate phrased as "suite X stays green" is only a gate if suite X exercises the thing being changed**, and that has to be verified when the gate is written, not when it fires. Rewriting `dashboard.spec.ts` against the rendered page also forced `role="group"` onto `KpiCard` and a named `<section>` onto `WidgetCard` — the same accessibility change the overview needed in August, for the same reason. |
| 2026-08-26 | **A ClickHouse select-list alias resolves inside `WHERE`**, so `SELECT toString(project_id) AS project_id … WHERE project_id IN {p:Array(UUID)}` matches nothing and raises nothing | Recorded because it is a defect class, not a typo. Every bucket and every per-project count on both dashboards returned empty: the comparison had become `String` against `Array(UUID)`, which is valid SQL with a wrong answer. A type checker cannot see it, a unit test asserting on SQL text cannot see it, and the e2e suite as it then stood could not see it either — **only the integration suite did**, because it is the only place a query runs against a server with known data and a literal expected number. That is the argument for keeping those 156 tests, and it is the third time in this repository that a silently-wrong query survived a green suite. The fix is a rule stated in the service: never alias a converted column back to its own name — and none of the conversions was needed, since `JSONEachRow` renders `UUID`, `Enum8` and `IPv6` as strings already. |
| 2026-08-26 | **The environment filter list goes back to a 30-day scan**, undoing the registry table added 2026-08-20 — and the argument for it is unmeasured on purpose | The registry existed because that exact scan was **13.4% of the overview's database time** in Postgres, so this reverses a decision with a number behind it. The case for reversing: `environment` is `LowCardinality`, so the scan reads one dictionary-encoded column; Phase 5's `p_minute` projection carries `environment` in its key, at which point the optimizer answers it from the aggregate without the application knowing; and keeping a Postgres table maintained by ingest to avoid a ClickHouse `GROUP BY` is precisely the machinery §1.2 counts as the cost of the old design. **The case against is that none of that is measured yet**, and it is written down here as an argument rather than a result. The benchmark exists and is named for it. If the scan is expensive the answer is the projection, not the registry — but that is a prediction too, and Phase 5 is where both get checked. |
| 2026-08-26 | **Disposable databases are dropped and recreated rather than reused** — the integration and e2e ones; the dev one is the developer's problem, stated loudly | The bill for having no migration chain arrived one phase after the decision. An end-state schema file is applied **additively**: it creates what is missing and leaves what exists alone, so a table *removed* from the schema lives on for ever. Phase 4 removed six, and `events` — with its `ON DELETE RESTRICT` foreign key to `projects` — then broke `resetDb()` in every e2e spec with a constraint violation over rows nothing writes any more. Correct code, correct file, wrong database, and nothing in the error mentions schema. For a database that is seeded from scratch anyway, tearing it down costs a second and removes the whole class. For the dev database it is not automatable safely, so it is documented in two reference files and in `PROGRESS.md` instead: `down -v`, then `db:bootstrap`. **This does not reopen the migrations decision** — its stated condition is an install holding data somebody depends on, which is still not the case — but it is the first concrete instance of what that decision costs, and it belongs beside it. |
| 2026-08-26 | **ClickHouse query-building lives in `core/clickhouse/`, not in a feature's `services/`** — one filter compiler and one search parser, called by both `features/events` and `features/alerts` | `PROJECT.md` §7 puts data access in `services/` and that still holds: `listEvents` and `countMatchingEvents` are services. What moved underneath them is turning an `EventFilters` into a bound `WHERE` clause, and two features need exactly that. §2.1 forbids one importing the other, so a compiler inside either would have forced the duplication it exists to remove — and that duplication was already there: the alert evaluator held a second copy of the events page's clause builder, eleven fields written twice, with nothing comparing them. That is the same shape that gave `topMessages` two implementations and two answers. There is a second reason and it is the stronger one: with no Drizzle dialect, parameter binding stopped being a library guarantee and became a rule somebody has to follow. A rule spread over every service that queries `events` is a rule that will be broken once and not noticed; in one file it is a question with a test. **This is not a licence to move feature logic into `core/`** — the test is "would placing it in a feature create a cross-feature import", not "might it be reusable". |
| 2026-08-26 | An attribute filter **compares as text** (`toString(getSubcolumn(…)) = 'v'`), replacing Postgres' type-strict `attributes @> '{"k":"v"}'::jsonb` | Not a concession to the new engine — a bug fix. The URL only ever carries strings, so a filter on a **numeric** attribute (`retries=2`) built a jsonb containment against `"2"` and matched a stored `2` never, silently, with no error and no empty-state hint that the filter was incapable rather than unmatched. Comparing the text forms makes them agree. The cost is that a string `"2"` and a number `2` become indistinguishable through this filter; the value shown in the UI still carries its type, and nobody has asked to filter on the difference. One case needs care and gets it: `toString` of an absent path is also `''`, so a filter for the empty string additionally asserts `dynamicType(...) != 'None'` — otherwise "the key is present and blank" would silently mean "the key is missing". |
| 2026-08-26 | The `innerJoin(projects) WHERE deleted_at IS NULL` defence in depth is **kept as a concurrent Postgres lookup** rather than dropped with the join | `security.md` had already written this off as an unavoidable casualty of the second store, and it would have been an easy thing to lose quietly — both callers resolve the project through `getProjectBySlug` first, so nothing would have broken today. That is exactly the argument that makes a second line of defence disappear: it is only ever load-bearing on the day the first one is wrong. A primary-key lookup issued in the same `Promise.all` as the ClickHouse query costs no added latency on a page already waiting for the larger one, which makes "keep it" cheap enough that dropping it would have to be argued rather than assumed. The referential integrity — the foreign key itself — is genuinely gone and is recorded as a gap. **The general rule this instances:** when a property cannot survive a migration in its original mechanism, the choice is to reimplement it or to record its loss, never to let it lapse because nothing failed. |
| 2026-08-26 | **Ingest dual-writes to ClickHouse and Postgres** for the length of Phases 2–4, against `09-clickhouse.md` §12's "no dual-write" | That line is about *data migration*, and it stays true — there is no existing data, so no dual-write period is needed to cut over. This is a different thing. The read path does not move until Phases 3 and 4, so writing only to ClickHouse would leave the events list and every dashboard reading an empty Postgres table for roughly a week, with the e2e suite red throughout — and a real regression introduced in Phase 3 would be indistinguishable from the breakage put there on purpose. §12's own gate for Phase 2 ("ingest e2e green", not "e2e green") is the tell that it had accepted this. Ten lines buys the whole suite staying green at every commit. It also buys something the plan did not consider: both stores hold the **same rows**, produced by one `enrichEvent` call, so a rewritten ClickHouse read can be checked against the Postgres one it replaces on identical data — which matters in a repository that has shipped green tests over broken code three times. The cost is named rather than hidden: no transaction spans the two stores, so a request failing after the ClickHouse write leaves the event in one and not the other. Phase 4 deletes the Postgres half; the scaffolding has an end date, not an intention. |
| 2026-08-26 | **Deduplication requires a caller-supplied `Idempotency-Key`; a token derived from the payload is not merely worse, it is unbuildable** | `09-clickhouse.md` §10 said "a token derived from the batch". Measuring it (§14.8) showed `insert_deduplication_token` **wins over the block checksum** — a second insert carrying a token ClickHouse has seen is discarded *whatever it contains*. So a content-derived token discards two genuinely different requests whenever their bodies match, and for a logging service they match constantly: a heartbeat, a retry loop, the same error twice in a second. `{"level":"info","message":"tick"}` sent twice is two events. Nor can the hash include anything server-side — the id and the arrival time are exactly what separate those two requests, and equally what separate a retry from its original, so a token containing either deduplicates nothing. Only the caller knows. Absent the header the behaviour is today's: duplicates stored, visible, countable. **Silently losing events is strictly worse than storing a duplicate somebody can see** — the same trade §10 made when it chose `wait_for_async_insert = 1`. The same measurement caught a second thing: on a plain `MergeTree` the token is accepted and silently ignored unless `non_replicated_deduplication_window` is set, and Phase 1 had shipped without it, so the feature as specified would have done nothing at all. |
| 2026-08-26 | Client-supplied values that a ClickHouse column cannot hold are **normalised, never rejected** — a blank optional string becomes absent, an unparseable `X-Forwarded-For` becomes `::` | Two schema facts force a decision here and §4.1 offered both answers without choosing. The table has **no `Nullable` column**, so `""` and absent are the same row; and `IPv6` rejects an unparseable address by failing the **entire insert** (code 676), which for a batch is 500 events lost to one malformed proxy header. Rejecting would mean returning 400 because a caller sent `""` for a field it never had to send, or because something upstream wrote `1.2.3.4:5678` into a header the caller never set — discarding an event to protect a facet list. For an ingest endpoint a dropped event is the worse failure every time, and the discarded values carry no information either way. The same principle already governs the timestamp policy, which coerces a future timestamp rather than refusing it. |
| 2026-08-26 | **ClickHouse enters the stack as a second store** — Phase 1 of `features/09-clickhouse.md` ships the `events` table, the container, the client, the env schema and the healthcheck. Nothing reads or writes it yet | The performance case for this migration **did not fire and is not the reason**: a cold 30-day dashboard measured 437 ms. Three other things are. Retention is becoming unbounded and a year of Postgres does not fit the disk at any setting. ~1,450 lines exist solely to work around row storage, and nearly every one has produced a silently wrong answer at least once. And three new requirements — filters on almost every field, per-project attribute shapes, and grouping by an arbitrary property — cannot be pre-aggregated at all: you cannot build a rollup for a group-by you do not know in advance. Phase 1 is deliberately inert. The two irreversible choices (`ORDER BY`, `PARTITION BY`) were settled by loading two candidate tables and measuring, not by argument, and shipping the schema first means the write path in Phase 2 has something already proven to write into. |
| 2026-08-26 | **The migration system is deleted.** `core/db/migrations/0000`–`0015`, `migrate.ts`, `migration-status.ts`, `scripts/apply-migrations.mjs` and `scripts/migrate-e2e.mjs` are gone; each store gets one file describing its end state, applied from empty by `core/db/bootstrap.ts` | Asked for directly, and the timing is what makes it defensible rather than reckless. Three facts hold **right now and not for much longer**: no install holds data worth keeping (the staging host was destroyed, which is also why this migration needs no dual-write and no backfill); the Postgres schema is about to *lose* its largest table and everything derived from it, so four of the sixteen migrations exist only to build things Phase 4 deletes; and a chain is a record of how a database got somewhere, which is worth exactly nothing when every database is created empty. Against that, a sixteen-file chain has to be replayed on every new test database and read in full by anyone asking what a table looks like. **The cost is real and is not deferred quietly:** a schema change against a database holding rows now has no upgrade path — the bootstrap creates what is missing and leaves what exists alone, so a rename or a retype will silently not apply. That is acceptable while the answer to "what would we lose" is "nothing", and stops being acceptable at the first install anybody depends on. **That is the condition for reopening this**, and it is written into `OPERATIONS.md`'s rollback section rather than left here, because that is where somebody will be standing when it matters. |
| | | **The Postgres half is generated, not hand-written.** `scripts/build-schema.mjs` runs `drizzle-kit export` — a diff against an empty database — so `core/db/schema/*.ts` stays the single source of truth and the `.sql` cannot drift from it. Two things the generator cannot do are done around it: every `"events"` statement is stripped and the hand-written `db/events.sql` spliced in (partitioning, the composite PK it forces, two GIN indexes and the pg_partman registration are outside Drizzle's DSL), and each statement is made idempotent. **Splicing rather than hand-editing is the whole point**: migration 0003 was a hand-edit inside a generated file and survived only because nothing ever regenerated it. Both halves are unit-tested, including that the `"events"` filter matches the *quoted* identifier — an unquoted `/events/` would silently take `event_rollup_minutes` and `event_template_rollup` with it, and the baseline would lose two tables with no error until a query failed at runtime. |
| | | **`drizzle-kit push` is removed too, not kept as a dev convenience.** It would create `events` as an ordinary table, and the bootstrap's `CREATE TABLE IF NOT EXISTS` would then skip the partitioned definition without complaining — a silently wrong schema, produced by the shortcut. One mechanism, not two. |
| | | **The readiness probe lost its `migrations` check and gained a ClickHouse ping.** Nothing replaces the migration check: it asked "is the schema current", and with the schema applied by a one-shot container that compose gates `app` on, an app that is serving at all has already had it succeed. The check moved out of a runtime probe and into the boot order, which is a better place for it. The ClickHouse ping is fatal rather than a warning for the same reason the Postgres one is — from Phase 2 on, an unreachable ClickHouse means ingest drops events and every event view is empty. |
| | | **The e2e database gap closed as a side effect, and the fix is not where it was expected to be.** `PROGRESS.md` had carried it for two weeks as "a line in package.json". Wiring it as Playwright's `globalSetup` was tried first and does not work: Playwright starts `webServer` **before** `globalSetup`, so the dev server comes up, Playwright polls the base URL, `proxy.ts` queries `users`, and the run dies with `database "logger_test" does not exist` before setup has run at all. It is `scripts/bootstrap-e2e.mjs` chained into `npm run test:e2e` instead. Worth recording because the obvious placement is the wrong one. |
