...
|---|---|---|
| 2026-08-25 | **A command that runs against a throwaway database migrates it; a command that measures one only checks it.** `test:e2e` now migrates `logger_test` before Playwright starts; `bench` refuses a `logger_bench` that is behind, and never writes | Both databases drifted on the same day and neither said so. `logger_test` five migrations behind surfaced as `Ingest failed: 500` in five specs — an error pointing at code that was fine; `logger_bench` behind surfaced as *nothing at all*, silently taking the raw-events path and producing numbers that would have been quoted. WORKFLOW.md §3.1 argues a constraint should live in the mechanism, and the repository already agreed with itself here: `logger_itest` is created, extended, migrated and seeded in code *specifically* as the answer to how `logger_test` was set up by hand. Nothing recorded a reason for keeping the e2e steps apart, and the measured cost of joining them is 0.33 s against a 2.3-minute suite, skipping the migrator entirely when nothing is pending. |
| | | The asymmetry is the part worth remembering. `bench` deliberately takes `DATABASE_URL` from the environment so the same benchmark can be pointed at staging over an SSH tunnel — so a setup step that migrated would run DDL against staging on `npm run bench`. Reading one row from `drizzle.__drizzle_migrations` is safe against any target; writing is not. The rule generalises: **automate the fix where the tool owns the database, automate only the detection where it does not.** Both paths share `scripts/migration-drift.mjs`, which reproduces the migrator's own comparison (journal `when` against the newest `created_at`) rather than counting rows, because a check that reports drift the migrator would not fix is worse than no check. |
