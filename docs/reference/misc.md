...
- **Unit/service tests**: Vitest (`vitest.config.ts` — jsdom environment, `@` alias to repo root, excludes `e2e/**` and `.next-e2e/**`). **63 test files / 726 tests** (recounted 2026-08-23), `npm run test`. All are for **services/utils/Server Action logic** — there are currently **no `*.test.tsx` component tests** despite `@testing-library/react` and `jest-dom` being installed. Coverage concentrates on the security/data-integrity-critical modules: `features/ingest` (6 files: API key auth, rate limiting, attribute-type registry, event schema, timestamp sanitization), `features/alerts` (evaluator, dispatcher, **CRUD/version/state-transition logic in `alert-rules.service.test.ts`**, and the SSRF URL guard in `webhook-url.test.ts`), **`features/auth` (all 9 Server Actions, added 2026-08-13)**, plus `shared/permissions/check.test.ts`.

  **`features/overview/` got its first tests on 2026-08-20** (`PLAN.md` §16.1 Stage B): **53 tests over its four `utils/` modules** — `overview-filters.ts`, `build-project-rows.ts`, `bucket-totals.ts` and `top-errors-window.ts` — plus 24 over `services/overview-cache.service.ts`. (A fifth, `overview-cache-key.ts`, moved to `shared/utils/query-cache-key.ts` on 2026-08-21 when the project dashboard needed the same key builder; its 21 tests went with it.) The first three were extracted from `app/[org]/(org-shell)/page.tsx` and `OverviewPage.tsx` in the same change. `overview.service.ts` itself — six raw-SQL aggregations — is covered by the integration suite below, not here: the query-builder mocking pattern cannot reach raw `db.execute`.
  ...
  **The db-mocking pattern does not cover the aggregation services.** All nine db-mocking tests stub the Drizzle *query builder* (`db.select().from().where()`). `overview.service.ts` and `aggregations.service.ts` use `db.execute(sql\`…\`)` with hand-written SQL instead, and stubbing `execute` to assert on the generated SQL text would test the string, not the result — it breaks on reformatting and passes on a semantically wrong query. That is what the integration tests below exist for. **`aggregations.service.ts` was covered on 2026-08-21** (`aggregations.service.itest.ts`, 26 tests), which is what made the two `ORDER BY` defects in it fixable — `PLAN.md` §17 had recorded them as "fixed only where a test can prove it", and until then nothing could.

- **Integration tests** (added 2026-08-20): Vitest against a **real Postgres**, `npm run test:it` (`vitest.integration.config.ts`, node environment). **4 files / 102 tests** (recounted 2026-08-21, after `aggregations.service.itest.ts` was added): `overview.service.itest.ts` (39), `aggregations.service.itest.ts` (26), `environment-registry.service.itest.ts` and `event-rollup.service.itest.ts`.
...
- **Benchmarks** (added 2026-08-20, `PLAN.md` §16.1 Stage C): `npm run bench` (`vitest.bench.config.ts`, `*.bench.ts`). Two files:

  - `features/overview/services/overview.service.bench.ts` — the six org-overview aggregations individually plus the whole page fan-out. `getProjectStats` and `getProjectTopMessages` are benched separately since the 2026-08-20 split; as one function the benchmark reported the maximum of the two, which hid the ~30× gap between them.
  - `features/dashboard/services/aggregations.service.bench.ts` (added 2026-08-21) — the project dashboard, which had never been measured. It benches the fan-out twice, once bare and once the way the route issues it (`hasAnyEvents` awaited first), because the gap between those two *is* the cost of that serialisation and was the only way to size it. The answer was 1.2 ms of a 170 ms page.

  Both discover their target at run time through `bench/support/target.ts`, and both anchor the range on the **newest event** rather than on `now()` — a corpus that stopped being written to would otherwise measure an empty window and report it as a fast query. `getProjectStats` and `getProjectTopMessages` are benched separately since the 2026-08-20 split — as one function the benchmark reported the maximum of the two, which hid the ~30× gap between them.
  ...
  - **`npm run bench` refuses to measure a database behind `core/db/migrations`** (`bench/support/global-setup.ts`, wired as `globalSetup` in `vitest.bench.config.ts`). It names the pending tags and stops before a single benchmark runs.

    Same 2026-08-25 trap as `logger_test`, different consequence. `bench:seed` migrates, `bench` never did, and `logger_bench` is seeded once and reused for weeks — so the corpus drifts behind the code by exactly as long as it has been since anyone rebuilt it. When it did, `event_rollup_minutes` was missing, every read fell back to scanning raw `events`, and the run produced numbers that looked entirely plausible while measuring the wrong query plan. A benchmark that is merely broken tells you it is broken; this one would have been *quoted*.

    **It checks and refuses rather than migrating, unlike `test:e2e` and `test:it`.** The two are not inconsistent: those own a disposable local database, whereas `DATABASE_URL` here is deliberately overridable so the same benchmark can be pointed at staging over an SSH tunnel (see the header comment in `vitest.bench.config.ts`). A setup step that migrates would then run DDL against staging on `npm run bench`, which is not a thing a measurement command may do. Reading one row from `drizzle.__drizzle_migrations` is safe against any target; writing is not. `bench/support/env.ts` resolves the URL once so the check and the benchmark cannot end up looking at different databases — a drift check pointed elsewhere is worse than none, because it reports "up to date" about something nobody measured.
...
- **E2E tests**: Playwright (`playwright.config.ts`), 12 spec files in `e2e/`: `alerts`, `api-keys`, `auth`, `auth-bootstrap`, `dashboard`, `events`, `ingest`, `invite`, `overview`, `projects`, `role-management`, `theme` — 73 tests total, run with `npm run test:e2e`.
  ...
  **Isolated e2e environment** — e2e runs against its own database and app instance, never the shared dev database:
  ...
  - **`npm run test:e2e` runs that migration itself** (`node scripts/migrate-e2e.mjs && playwright test …`). `npm run db:migrate:e2e` still exists and still does exactly the same thing; it is no longer something you have to remember.

    *Changed 2026-08-25, after the separate step cost an afternoon.* `logger_test` was five migrations behind — at 0009, missing `template_hash` and `by_source` on `events` — and the only symptom was five specs failing with `Ingest failed: 500 {"error":"Internal server error."}`. That points squarely at the ingest path, which was fine. Nothing in Playwright's output mentioned migrations, and `npm run test` and `npm run test:it` were both green, because one needs no database and the other rebuilds its own on every run. After migrating, 64 of 65 specs passed on the spot.

    The trade-off, stated so it does not have to be rediscovered: the migration adds **~0.33 s** to a suite that takes **~2.3 minutes**, and that is the *always* cost — the up-to-date path reads one row from `drizzle.__drizzle_migrations`, finds nothing pending, and skips the migrator entirely. There is no case where the old ordering was preferable: nobody wants to run e2e against a schema older than the code under test, and if they did, they would be running Playwright directly rather than the npm script. Weighed against a second afternoon, a third of a second is not a cost worth naming.

    Nothing in the repository recorded a reason for keeping the steps apart, and the surrounding design already argues the other way: `logger_itest` is created, extended, migrated and seeded in code (see the top of this section) *specifically* as an answer to how `logger_test` was set up by hand. This closes the same gap on the same database.

    Two consequences worth knowing. `scripts/migrate-e2e.mjs` now **exits 1 if `DATABASE_URL` is unset** instead of proceeding: on a checkout with no `.env.e2e.local`, postgres.js would otherwise fall back to its own defaults (localhost, the OS user, a same-named database) and migrate whatever that reached. That was harmless while the script was typed deliberately and is not harmless now that a test command runs it. And it prints what it did — `e2e database already at 0013_ambitious_sabra`, or the list of tags it applied — because a silent step is the thing this change exists to stop.
  ...
  - **`E2E_MODE=true` also collapses the overview read cache to a 1 ms TTL and a 1 ms staleness ceiling** (`readCacheSettings()` in `shared/utils/read-cache-settings.ts`). Live, those reads are held for 30 seconds and shared between viewers — which would make any spec that ingests events and then asserts on the overview read the *previous* spec's numbers, fail with figures that look plausible rather than wrong, and give no hint that a cache was involved. The specs as written happen to dodge it because each gets its own organization and the cache is keyed on the project scope; that is luck, not a property anyone would know to preserve. Both values shrink together on purpose: a 1 ms TTL under a five-minute ceiling would still serve stale for five minutes, just from the background-refresh branch instead of the fresh one. Be precise about what that leaves running: single flight and the ceiling stay in the path, **the stale-while-revalidate branch does not**. With `ttlMs === maxStaleMs` the fresh and expired tests partition every possible age, so `refreshInBackground` is unreachable under e2e. That is a real cost of this setting — the branch most likely to hide a bug is the one e2e no longer exercises — and it is accepted because determinism in the specs matters more, with the branch covered by `ttl-cache.test.ts` instead.
...
- Project convention (`.claude/rules/PROJECT.md`): unit/integration tests live next to the source file (`thing.ts` → `thing.test.ts`, `thing.itest.ts`), test behaviour via role/label queries not implementation details, and mock only at system boundaries. *Corrected 2026-08-20 — this line used to say `ComponentName.test.tsx`, a convention the repository has never used: there are zero `.test.tsx` files, as the top of this section says.*

## Continuous integration

Two workflows in `.github/`, both added 2026-08-13 (before that, nothing ran on push — every merge was verified by hand):

- **`ci.yml`** — runs on push to `main` and on every pull request. Job `gates` runs the four checks from `WORKFLOW.md` §4 in order (`tsc --noEmit`, `lint`, `test`, `build`), each with `if: always()` so one red gate does not hide the others. Job `image` builds the Docker image in parallel without pushing it, so a Dockerfile break is caught by CI rather than at deploy time. Both use Node 22, matching the image's base. `DATABASE_URL`/`AUTH_SECRET` are set to throwaway values because `npm run build` imports `core/env`, which validates the whole schema on import; no database is contacted.
...
Three scripts push events into a **running** install through the real ingest HTTP API. None of them touches the database directly — that is deliberate, because writing rows behind the API would exercise none of auth, rate limiting, validation or the attribute type registry. All three read `LOGGER_URL` and `LOGGER_API_KEY` from **`.env.local`**, which is gitignored; no key is ever committed, since this repository is public.

`event-one-by-key.mjs` and `events-batch-by-key.mjs` take an optional **key slot** as their first argument — `node scripts/event-one-by-key.mjs 2` reads `LOGGER_API_KEY_2` instead of `LOGGER_API_KEY`. Two slots exist because one generator drives one project, and an org overview only looks like a real one when more than a single project is producing traffic. `resolveApiKey()` in `event-factory.mjs` **throws on an unrecognised slot rather than falling back to slot 1**: a typo would otherwise send one project's load into another, and the events would look entirely plausible on arrival — surfacing as a puzzling dashboard days later instead of an error at the point of the mistake. `demo-live.mjs` reads slot 1 only, or `--key` for a one-off.

These three variables are deliberately **not** in the `core/env` Zod schema, unlike every runtime variable (`PROJECT.md` conventions). Nothing in the running app reads them; putting them there would fail the app at boot over a script it never runs. `.env.example` documents them instead.

> The keys moved out of the scripts on 2026-08-22, after one was pasted into `event-one-by-key.mjs` as a literal default — directly beneath the comment explaining why that must not happen. It was caught before any commit and the key never entered the history, but the fix is structural rather than a reminder: there is no longer a place in these files for a key to sit, and the file that does hold them cannot be committed.
...
| `event-one-by-key.mjs` | one request per event, rate oscillating over a configurable envelope (default 750–950/min) | watching the dashboard move — the shape of the curve is the point |
