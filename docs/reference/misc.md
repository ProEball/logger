...
- **Integration tests** (added 2026-08-20): Vitest against a **real Postgres**, `npm run test:it` (`vitest.integration.config.ts`, node environment). **4 files / 102 tests** (recounted 2026-08-21, after `aggregations.service.itest.ts` was added): `overview.service.itest.ts` (39), `aggregations.service.itest.ts` (26), `environment-registry.service.itest.ts` and `event-rollup.service.itest.ts`.
  ...
  - **Only one integration run at a time, per machine.** `logger_itest` is a fixed name with no per-checkout suffix, and `global-setup.ts` opens by wiping the corpus (`DELETE FROM projects` cascades to every rollup table). So a second checkout — a git worktree for an agent task, a second clone — running `npm run test:it` concurrently deletes the first run's fixture underneath it. Observed 2026-08-25 as three integration tests failing once and never again across eight further runs, while a background task was working in `.claude/worktrees/`. It looks like flakiness and is not: it is two suites sharing one database. If you need genuine concurrency, set `ITEST_DATABASE_URL` per checkout.
...
- **E2E tests**: Playwright (`playwright.config.ts`), 12 spec files in `e2e/`: `alerts`, `api-keys`, `auth`, `auth-bootstrap`, `dashboard`, `events`, `ingest`, `invite`, `overview`, `projects`, `role-management`, `theme` — 73 tests total, run with `npm run test:e2e`.
  ...
  **Isolated e2e environment** — e2e runs against its own database and app instance, never the shared dev database:
  ...
  - All three vitest configs exclude **`.claude/worktrees/**`**, where git worktrees for agent tasks live. Each is a full checkout, so without the exclusion one background task doubles the suite and reports that copy's failures as this one's — seen 2026-08-25 as "13 failed files" that were a single worktree sitting at an older commit.
  - **`npm run test:e2e` does not apply migrations, and a stale `logger_test` does not announce itself as stale.** Observed 2026-08-25: the database was five migrations behind (at 0009, missing `template_hash` and `by_source`), and the symptom was five specs failing with `Ingest failed: 500 {"error":"Internal server error."}` — an ingest-path error, pointing at code that was fine. Nothing in the output mentioned migrations. Run `npm run db:migrate:e2e` after **any** migration lands, and suspect it first whenever ingest fails wholesale in e2e while `npm run test` and `npm run test:it` are green. The same trap caught `logger_bench` the same day, where it silently made the template rollup unreachable.
