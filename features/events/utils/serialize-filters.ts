import type { EventFilters } from "./event-filters.types";

/**
 * Serialize EventFilters to URLSearchParams. Round-trip safe with parseFilters.
 */
export function serializeFilters(filters: EventFilters): URLSearchParams {
    ...
    if (filters.releases?.length) {
        params.set("releases", filters.releases.join(","));
    }

    if (filters.errorTypes?.length) {
        params.set("errorTypes", filters.errorTypes.join(","));
    }
