...
export function FilterBuilder({ value, onChange }: FilterBuilderProps) {
    ...
    const update = (patch: Partial<EventFilters>) => onChange({ ...value, ...patch });
    ...
    const hasAny = !!(
        value.levels?.length || value.environments?.length || value.sources?.length ||
        value.releases?.length || value.errorTypes?.length || value.message
    );

    return (
        <div className={styles.pile}>
            {value.levels?.length ? (
                <FilterChip filterKey={t("events.filters.level")} value={value.levels.join(", ")} variant="red"
                    onRemove={() => { hideFilter("level"); update({ levels: undefined }); }} />
            ) : null}
            {value.environments?.length ? (
                <FilterChip filterKey={t("events.filters.environment")} value={value.environments.join(", ")} variant="green"
                    onRemove={() => { hideFilter("environment"); update({ environments: undefined }); }} />
            ) : null}
            {value.sources?.length ? (
                <FilterChip filterKey={t("events.filters.source")} value={value.sources.join(", ")} variant="cyan"
                    onRemove={() => { hideFilter("source"); update({ sources: undefined }); }} />
            ) : null}
            {value.releases?.length ? (
                <FilterChip filterKey={t("events.filters.release")} value={value.releases.join(", ")} variant="purple"
                    onRemove={() => { hideFilter("release"); update({ releases: undefined }); }} />
            ) : null}
            {value.errorTypes?.length ? (
                <FilterChip filterKey={t("events.filters.errorType")} value={value.errorTypes.join(", ")} variant="orange"
                    onRemove={() => { hideFilter("errorType"); update({ errorTypes: undefined }); }} />
            ) : null}
            {value.message ? (
                <FilterChip filterKey={t("events.filters.message")} value={value.message}
                    onRemove={() => { hideFilter("message"); update({ message: undefined }); }} />
            ) : null}

            {shouldShow("level") && !value.levels?.length ? (
                <LevelFilter value={value.levels ?? []} onChange={(levels: EventLevel[]) => update({ levels })} />
            ) : null}
            {shouldShow("environment") && !value.environments?.length ? (
                <StringListFilter labelKey="events.filters.environment" value={value.environments ?? []} onChange={(environments) => update({ environments })} />
            ) : null}
            {shouldShow("source") && !value.sources?.length ? (
                <StringListFilter labelKey="events.filters.source" value={value.sources ?? []} onChange={(sources) => update({ sources })} />
            ) : null}
            {shouldShow("release") && !value.releases?.length ? (
                <StringListFilter labelKey="events.filters.release" value={value.releases ?? []} onChange={(releases) => update({ releases })} />
            ) : null}
            {shouldShow("errorType") && !value.errorTypes?.length ? (
                <StringListFilter labelKey="events.filters.errorType" value={value.errorTypes ?? []} onChange={(errorTypes) => update({ errorTypes })} />
            ) : null}
            ...
            {hasAny && (
                <button type="button" className={styles.clearBtn} onClick={() => { setOpenFilters(new Set()); onChange({ range: DEFAULT_RANGE }); }}>
                    Clear
                </button>
            )}
        </div>
